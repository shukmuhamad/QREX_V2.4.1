<?php
        if (isset($_POST['submit']))
        {
            ...
            $GloveSurfaceName       = $_POST['SurfaceName'];

            $query = "select * from M_GloveSurface where GloveSurfaceName = ?";
            $stmt = $connect->prepare($query);
            $stmt->bindParam(1, $GloveSurfaceName);
            $stmt->execute();
            $surfaceInfo = $stmt->fetch();

            $GloveSurface       = $surfaceInfo['GloveSurfaceCode'];
